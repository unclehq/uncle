...
t.stage_runners["derive-brief"] = "claude"
check("claude hides both", ["runner", "effort"],
      t.stage_fields("derive-brief"))
check("claude resolves to no model", "", t.stage_model("derive-brief"))
...
t = fresh()
t.stage_models["derive-brief"] = "cline-pass/glm-5.3"
t.stage_runners["derive-brief"] = "kimi"
t.save_config()
text = open(os.environ["UNCLE_CONFIG"]).read()
check("dormant model line survives", True, "derive-brief.model cline-pass/glm-5.3" in text)
...
fixtures = {
    ...
    "bare": "derive-brief poolside/laguna-s-2.1\nbilling cline-usage\n",
    "stage": "derive-brief.model poolside/laguna-s-2.1\nderive-brief.billing cline-usage\n",
    "precedence": (
        "model poolside/laguna-s-2.1\nbilling cline-usage\n"
        "reviewer cline-pass/glm-5.3\n"
        "derive-brief cline-pass/kimi-k3\n"
        "derive-brief.model poolside/laguna-s-2.1\n"
        "derive-brief.billing clinepass\n"
    ),
}
checks = 0
for name, config in fixtures.items():
    ...
    if name == "stage":
        expected_billings = {"derive-brief": "cline-usage"}
    elif name == "precedence":
        expected_billings["derive-brief"] = "clinepass"
        assert t.stage_models["derive-brief"] == "poolside/laguna-s-2.1"
...
for runner in ("cline", "claude", "kimi", "codex", "self-hosted"):
    with open(os.environ["UNCLE_CONFIG"], "w") as fh:
        fh.write("derive-brief.runner %s\nderive-brief.model poolside/laguna-s-2.1\n"
                 "derive-brief.billing cline-usage\n" % runner)
    t = load()
    expected_models = dict(t.stage_models)
    t.save_config()
    t = load()
    assert t.stage_runner("derive-brief") == runner
    assert t.stage_models == expected_models, (runner, "dormant model loss")
    assert t.stage_billings == {"derive-brief": "cline-usage"}, (runner, "billing loss")
    checks += 3
...
actual="$(
    UNCLE_CONFIG="$TMP/proj/.uncle/config" ROOT="$ROOT" bash -c '
        ...
        for stage in derive-brief project-plan change-plan adversarial-review \
                     updated-plan updated-change-plan preflight implementation repair \
                     test-review manual-checklist execute-checklist \
