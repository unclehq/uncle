...
The driver runs the plan's verification commands twice -- once now, against the
...
the plan will freeze during verification. Keep generated test outputs outside
...
required report to be written once after evidence is collected.
...
`.uncle/workflow/project-plan-investigation.md`, using these exact section
headings so the second pass can extract them mechanically:

- `## Verification commands`, holding one fenced `sh` block and nothing else
  -- see the requirements above.
- Everything else (the numbered sections, both tables, the implementation
  steps with their `Owns:`/`Depends on:` fields) as ordinary Markdown
  sections; the second pass carries all of it into the final document
  unchanged.

Do not put a `## Verification commands` heading anywhere except that one
section.

Write only `.uncle/workflow/project-plan-investigation.md`, in a single Write
call, then stop -- except for the `.gitignore` write above, which belongs in
this pass.
...
Define each verification command once in its required location. Strategy and
step rows reference those commands and check IDs; avoid second lists that can
diverge. Before the single final write, reconcile coverage, traceability,
dependencies and commands once. Resolve actual omissions or contradictions
without narrating repeated compliance checks. Write the complete plan, with no
preamble, work diary, filename-only answer or closing recap.
