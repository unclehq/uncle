...
class ChatInteractionTests(unittest.TestCase):
    ...
    def test_composer_text_starts_after_prompt_without_padding(self):
        ...
        self.ui.chat_cursor = len(self.ui.chat_composer)
        self.ui.stdscr = Mock()
...
class ChatCursorHistoryTests(unittest.TestCase):
    """Issue 70: composer cursor movement and chat-history recall."""
    ...
    def test_up_recalls_history_and_down_restores_draft(self):
        # AC-1, AC-2
        self.ui.chat_history = ['first', 'second']
        self.ui.chat_composer = 'draft'
        self.ui.chat_cursor = 5
        self.ui.handle_key(tui.curses.KEY_UP)
        self.assertEqual(self.ui.chat_composer, 'second')
        self.assertEqual(self.ui.chat_cursor, len('second'))
        self.assertEqual(self.ui.chat_saved_draft, 'draft')
        self.ui.handle_key(tui.curses.KEY_UP)
        self.assertEqual(self.ui.chat_composer, 'first')
        self.assertEqual(self.ui.chat_cursor, len('first'))
        self.ui.handle_key(tui.curses.KEY_DOWN)
        self.assertEqual(self.ui.chat_composer, 'second')
        self.ui.handle_key(tui.curses.KEY_DOWN)
        self.assertEqual(self.ui.chat_composer, 'draft')
        self.assertEqual(self.ui.chat_history_index, -1)
        self.assertEqual(self.ui.chat_cursor, len('draft'))

    def test_empty_history_up_down_are_noops(self):
        # CHANGE_SPEC §9: empty history is a no-op.
        self.ui.chat_history = []
        self.ui.chat_composer = 'draft'
        self.ui.chat_cursor = 3
        self.ui.handle_key(tui.curses.KEY_UP)
        self.ui.handle_key(tui.curses.KEY_DOWN)
        self.assertEqual(self.ui.chat_composer, 'draft')
        self.assertEqual(self.ui.chat_cursor, 3)

    def test_backspace_at_cursor(self):
        # AC-3 regression test: baseline deletes the last char ('ab'); the
        # fix deletes the char before chat_cursor ('bc'), per CHANGE_PLAN §1's
        # formula chat_composer[:chat_cursor-1] + chat_composer[chat_cursor:].
        self.ui.chat_composer = 'abc'
        self.ui.chat_cursor = 1
        self.ui.handle_key(tui.curses.KEY_BACKSPACE)
        self.assertEqual(self.ui.chat_composer, 'bc')
    ...
    def test_left_right_clamped(self):
        ...
        self.assertEqual(self.ui.chat_cursor, 2)
        self.ui.chat_cursor = 0
        self.ui.handle_key(tui.curses.KEY_LEFT)
        self.assertEqual(self.ui.chat_cursor, 0)

    def test_home_moves_to_zero(self):
        # AC-5
        self.ui.chat_composer = 'abcdef'
        self.ui.chat_cursor = 4
        self.ui.handle_key(tui.curses.KEY_HOME)
        self.assertEqual(self.ui.chat_cursor, 0)
        self.ui.chat_cursor = 4
        self.ui.handle_key(1)  # Ctrl-A
        self.assertEqual(self.ui.chat_cursor, 0)

    def test_end_moves_to_len(self):
        # AC-6
        self.ui.chat_composer = 'abcdef'
        self.ui.chat_cursor = 2
        self.ui.handle_key(tui.curses.KEY_END)
        self.assertEqual(self.ui.chat_cursor, 6)
        self.ui.chat_cursor = 2
        self.ui.handle_key(5)  # Ctrl-E
        self.assertEqual(self.ui.chat_cursor, 6)

    def test_word_backward(self):
        # AC-7
        self.ui.chat_composer = 'one two three'
        self.ui.chat_cursor = len(self.ui.chat_composer)
        self.ui.handle_key(tui.curses.KEY_SLEFT)
        self.assertEqual(self.ui.chat_cursor, len('one two '))
        self.ui.chat_cursor = len(self.ui.chat_composer)
        self.ui.handle_key(2)  # Ctrl-B fallback
        self.assertEqual(self.ui.chat_cursor, len('one two '))
    ...
    def test_word_forward(self):
        ...
        self.ui.chat_composer = 'one two three'
        self.ui.chat_cursor = 0
        self.ui.handle_key(tui.curses.KEY_SRIGHT)
        self.assertEqual(self.ui.chat_cursor, len('one '))
        self.ui.chat_cursor = 0
        self.ui.handle_key(6)  # Ctrl-F fallback
        self.assertEqual(self.ui.chat_cursor, len('one '))

    def test_cursor_render_position_viewport(self):
        # AC-9, AR-003: >6 wrapped chunks, cursor in a non-last visible chunk.
        renderer_state(self.ui)
        self.ui.state = 'running'
        self.ui.chat_focus = 'chat'
        self.ui.home_menu_open = False
        words = ['w%dxxxxxx' % i for i in range(10)]
        self.ui.chat_composer = ' '.join(words)
        prefix_text = ' '.join(words[:7])
        self.ui.chat_cursor = len(prefix_text)
        self.ui.stdscr = Mock()
        self.ui.stdscr.getmaxyx.return_value = (40, 160)
        self.ui._draw_chat_composer(25, 39, 160, 25, 10)
        calls = self.ui.stdscr.addnstr.call_args_list
        cursor = next(call for call in calls
                      if call.args[2] == ' ' and call.args[4] & tui.curses.A_REVERSE)
        composer_row = 27  # row(25) -1 for build+compact, +3 for the composer line
        viewport_start = 4  # 10 chunks, viewport keeps the last 6
        self.assertEqual(cursor.args[0], composer_row + (6 - viewport_start))
        self.assertEqual(cursor.args[1], min(25 + 2 + 8, 25 + 10 - 1))  # existing right-edge clamp
        self.assertNotEqual(cursor.args[0], composer_row + 5)  # not the last visible chunk

    def test_typing_exits_history_mode(self):
        # AC-10
        self.ui.chat_history = ['old']
        self.ui.chat_composer = ''
        self.ui.chat_cursor = 0
        self.ui.handle_key(tui.curses.KEY_UP)
        self.assertEqual(self.ui.chat_history_index, 0)
        self.ui.handle_key(ord('x'))
        self.assertEqual(self.ui.chat_history_index, -1)

    def test_history_and_cursor_work_on_home_and_build_pages(self):
        # AC-11
        for state in ('menu', 'running'):
            with self.subTest(state=state):
                self.ui.state = state
                self.ui.chat_focus = 'chat'
                self.ui.chat_history = ['alpha', 'beta']
                self.ui.chat_history_index = -1
                self.ui.chat_composer = 'draft'
                self.ui.chat_cursor = 5
                self.ui.handle_key(tui.curses.KEY_UP)
                self.assertEqual(self.ui.chat_composer, 'beta')
                self.ui.handle_key(tui.curses.KEY_LEFT)
                self.assertEqual(self.ui.chat_cursor, len('beta') - 1)

    def test_picker_and_non_chat_bindings_still_work(self):
        # AC-12: B-12 picker, B-13 prompt_scroll, B-14 build-scroll KEY_END.
        self.ui.state = 'running'
        self.ui.chat_focus = 'chat'
        self.ui.prompt_kind = ''
        self.ui.chat_choices = ['a', 'b', 'c']
        self.ui.chat_pick = 0
        self.ui.handle_key(tui.curses.KEY_DOWN)
        self.assertEqual(self.ui.chat_pick, 1)
        self.ui.chat_choices = []
        self.ui.prompt_kind = 'confirm'
        self.ui.chat_focus = 'gate'
        self.ui.prompt_scroll = 0
        self.ui.handle_key(tui.curses.KEY_DOWN)
        self.assertEqual(self.ui.prompt_scroll, 1)
        self.ui.prompt_kind = ''
        self.ui.chat_focus = 'chat'
        self.ui.build_page_rows = 5
        self.ui.build_scroll = 3
        self.ui.handle_key(tui.curses.KEY_END)
        self.assertEqual(self.ui.build_scroll, 3)
        self.ui.chat_focus = 'gate'
        self.ui.handle_key(tui.curses.KEY_END)
...
class ChatStylingTests(unittest.TestCase):
    ...
    def test_build_scroll_keys_leave_composer_intact(self):
        ...
        ui.handle_key(tui.curses.KEY_PPAGE)
        # AR-006: KEY_END is guarded by chat_focus so the composer can claim
        # it (AC-12); build-scroll only resets when chat is not focused.
        ui.chat_focus = 'chat'
