...
document; your only job here is to get the plan's content and every decision
right. Write your complete plan as plain Markdown -- not JSON -- to
`.uncle/workflow/updated-plan-investigation.md`, using these exact section
headings so the second pass can extract them mechanically:

- `## Verification commands`, holding one fenced `sh` block and nothing
  else -- see the requirements for it below.
- `## Protected verification paths`, holding one fenced `text` block and
  nothing else -- see the requirements for it below.
- `## Adversarial review dispositions`, holding one table with columns
  `Finding`, `Disposition`, `Reason`, `Exact plan change` -- one row per
  adversarial finding, described below.
- Everything else -- behaviors, invariants, architecture, traceability,
  testing strategy, implementation steps, restrictions, and so on -- as
  ordinary Markdown sections, in any order that reads as one coherent
  document.
...
`.uncle/workflow/updated-plan-investigation.md` already exists from an
interrupted attempt, read it and complete the remaining work, validating it
against current inputs. Do not restart the document from scratch.

Write one disposition row per adversarial finding: `Finding` is its AR-XXX
ID, `Disposition` is exactly one of `Accepted`, `Partially accepted`,
`Rejected`, `Deferred`, `Reason` is a concise sentence, `Exact plan change`
names the affected section/row (or states none was needed for a Rejected
finding). Do not blindly accept every recommendation.
...
Put in the `## Verification commands` block, as plain text with no prompt
prefixes and nothing else, the commands that demonstrate the build is
...
- no comments, no prose, no placeholders;
...
Run independent test suites in parallel. Add a `## Parallel verification
groups` section for all checks proven independent: no shared ports, writable
fixtures, outputs, or prerequisite ordering. The section holds one fenced
`text` block and nothing else — bare rows of one-based positions from the
Verification commands block (the numbering of the lines in that block), one
group per line, each row at least two consecutive numbers, rows ordered and
disjoint:
...
Put in the `## Protected verification paths` block, as plain text with no
comments and nothing else: literal repository-relative file or directory
paths, one per line:
...
Write only `.uncle/workflow/updated-plan-investigation.md`. Preserve a
complete standalone plan and stop.
...
6. Before the single final write, reconcile dispositions, traceability,
   ...
   diary, file-name-only answer or closing recap. Write the complete plan.
