...
class HeadlessTests(Fixture):
    ...
    def test_t2_attended_rerun_with_ledger_publishes(self):
        ...
        self.assertEqual((self.creates(), self.pushes(), self.commits()), ([], [], []))
        self.human_commit(result.stdout)
        self.ok(self.complete('', unattended=False, relay=False))
        self.assert_created()
    ...
    def test_t9_open_pipe_never_waits(self):
        ...
        self.assertEqual(self.journal()['phase'], 'bound')
        # A prepared journal whose commit the person has not made yet: the
        # dialog would wait on the pipe; the headless probe declines instead.
...
class PersonChannelTests(Fixture):
    def test_t3_auto_relay_unsigned_publishes_by_human_input(self):
        ...
        self.assertNotIn(SKIP, result.stdout)
        self.assert_prompted(result.stdout, signed=False)
        self.assertIn(PENDING_EOF, result.stdout)
        ...
        self.assertEqual(sorted({e['reason'] for e in events}), ['publication', 'signing'])
    ...
    def test_t6_not_ready_override_asks_the_person(self):
        ...
        self.assert_prompted(approved.stdout, signed=False)
        self.assertEqual(self.journal()['phase'], 'prepared')
        self.human_commit(approved.stdout)
        self.ok(self.complete('', unattended=True, relay=True))
        self.assert_created()
