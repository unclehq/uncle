...
.uncle/docs/UPDATED_PROJECT_PLAN.md. Investigate freely with your tools, before
implementation.

`narrative` covers, in order: Summary, Findings, Assumptions, Open questions
-- everything that is not the acceptance gate table itself. In Findings,
inventory every mandatory acceptance check and its prerequisites: commands
and dependencies, browser binaries and interaction access, source rendering,
credentials or services, representative input data, and independent
reviewers where required. Probe available capabilities and record exact
commands and results. Do not run future implementation commands whose files
do not exist yet; check that their execution environment exists.
...
The acceptance gate is one entry per prerequisite. IDs must be plain
identifiers such as `G-1`, with the label in the evidence, not in the ID.
Status is exactly one of `PASS`, `FAIL`, `BLOCKED-SETUP`, `BLOCKED-HUMAN`,
`BLOCKED-IMPOSSIBLE`, `NOT RUN`, `N/A`.
...
Write the pre-implementation input files that `Files the plan says to
create` requires you to derive. Nothing else.
...
document; your only job here is to get the probes, the derived files, and
the gate rows right -- and to actually derive the files above, since that
real Write-tool work belongs here, not in the formatting pass. Write your
complete report as plain Markdown -- not JSON -- to
`.uncle/workflow/preflight-investigation.md`, using these exact section
headings:

- `## Summary`, `## Findings`, `## Assumptions`, `## Open questions` --
  everything that is not the acceptance gate table itself.
- `## Acceptance gate`, holding one Markdown table with columns `ID`,
  `Required`, `Status`, `Evidence` -- one row per prerequisite, `Required`
  as `YES`/`NO`.

Begin the file with a Markdown heading, such as `# Preflight investigation`
-- a response with no heading anywhere in it is rejected as not a document
at all, regardless of whether its content is otherwise correct. Write it
