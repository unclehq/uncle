...
def config_stage(stage, values):
    ...
    if stage == 'updated-plan-investigate':
        return 'updated-plan'
    if stage == 'adversarial-review-investigate':
        return 'adversarial-review'
    if stage == 'manual-checklist-investigate':
        return 'manual-checklist'
    if stage == 'final-audit-investigate':
        return 'final-audit'
...
def reviewer_packet(response, output):
    ...
    if payload.get('kind') == 'test-review-worker-packet':
        ...
        for index, row in enumerate(rows, 1):
            if PLACEHOLDER_PACKET_ID.match(row['id'].strip()):
                raise InvalidReviewerDocument('Reviewer test-review row %d needs a stable finding ID, not placeholder %r for %s'
                                              % (index, row['id'].strip(), Path(output).name))
